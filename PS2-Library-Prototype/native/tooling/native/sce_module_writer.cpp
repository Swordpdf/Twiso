...
struct ParameterBlocks
{
    ...
    std::size_t flexible_size{};
    std::size_t internal_memory_size{};
    std::size_t max_system_size{};
};

ParameterBlocks build_parameter_blocks()
{
    constexpr std::array<std::size_t, 6> sizes = {0xa8, 0x38, 0x10, 0x78, 0xc0, 0x38};
    // counts[0]: entry_count=0x0e (14), SceLibcInternalHeap=0 (use flexible heap, not the
    // small kernel internal-memory pool). Matches real game proc params (e.g. Clair Obscur)
    // which leave libc_param unset entirely; the default heap is the large flexible one.
    constexpr std::array<std::uint64_t, 6> counts = {0x000000000000000e, 0, 0, 2, 3, 1};
    ...
    result.flexible_size = cursor + 16;
    result.internal_memory_size = cursor + 24;
    result.max_system_size = cursor + 32;
    result.data.resize(cursor + 40);
    for (std::size_t i = 0; i < sizes.size(); ++i)
    {
    ...
    // sceKernelFlexibleMemorySize: total flexible = value + 64MB base. Ask for 4GB.
    write_u64(result.data, result.flexible_size, 0x100000000ULL - 0x4000000ULL);
    // sceKernelInternalMemorySize: kernel internal memory available to the app.
    write_u64(result.data, result.internal_memory_size, 0x100000000ULL);
    // sceLibcMaxSystemSize: max memory areas for mspace.
    write_u64(result.data, result.max_system_size, 0x100000000ULL);
    return result;
}
...
Bytes write_executable(const Image &image, std::span<const Stub> stubs, const Options &options)
{
    ...
    const std::array<std::pair<std::uint64_t, std::uint64_t>, 11> parameter_pointers = {{
        ...
        {blocks_address + 0x58, blocks_address + blocks.internal_memory_size},
        {blocks_address + 0x68, blocks_address + blocks.max_system_size},
        {blocks_address + blocks.offsets[1] + 0x10, blocks_address + blocks.flexible_size},
    }};
    ...
    // The GOT can follow .data.rel.ro. Its file offset is not necessarily the
    // beginning of the RELRO mapping. Preserve the input VA-to-file mapping;
    // otherwise the emitted PT_LOAD is misaligned and maps the wrong bytes.
    require(got.address >= relro_start && got.file_offset >= got.address - relro_start,
            "LLVM GOT layout cannot describe the RELRO file mapping");
    const std::uint64_t relro_file = got.file_offset - (got.address - relro_start);
    require(relro_file % kPage == relro_start % kPage,
            "LLVM RELRO file offset and address are not page-congruent");
    for (const Section &input : image.sections)
    {
        if (input.allocated() && !input.no_bits() && input.size != 0 && relro_section(input.name))
            require(input.file_offset == relro_file + input.address - relro_start,
                    "LLVM RELRO sections have inconsistent VA-to-file mappings");
    }
    ...
    {
        const ProgramHeader &header = headers[i];
        if (header.type == kProgramLoad)
            require(header.offset % kPage == header.address % kPage,
                    "generated PT_LOAD has a non-page-congruent file mapping");
